...
public final class PinballBoard {
    ...
    private boolean launched,drained,tilted,loopArmed;
    ...
    public PinballBoard(Random random) {
        launchX=-65-random.nextDouble()*30;launchY=-940-random.nextDouble()*35;
    }
    ...
    private void step(boolean leftDown,boolean rightDown) {
        ...
        double next=move(left,leftDown&&!tilted?-.55:.35,(leftDown&&!tilted?18:8)*STEP);
        leftOmega=(next-left)/STEP;left=next;
        next=move(right,rightDown&&!tilted?-.55:.35,(rightDown&&!tilted?18:8)*STEP);
        rightOmega=(next-right)/STEP;right=next;
        vy+=620*STEP;vx*=.99985;
        limitSpeed();x+=vx*STEP;y+=vy*STEP;
        for(Rail rail:RAILS) segment(rail,2,0,0,.72,0);
        for(int i=0;i<SLINGS.size();i++) {
            int side=i/3;
            if(segment(SLINGS.get(i),3,0,0,.85,tilted?0:100) && !tilted && slingCooldown[side]<=0) {
                points(35);slingCooldown[side]=.2;
            }
        }
        for(int i=0;i<3;i++) {
            if(circle(BUMPERS.get(i),.92,tilted?0:230)) {
                if(!tilted && bumperCooldown[i]<=0) { points(100);bumperHits++;bumperFlash[i]=1;bumperCooldown[i]=.16; }
            }
            if(circle(TARGETS.get(i),.8,tilted?0:60) && !tilted && targetCooldown[i]<=0) {
    ...
    private void limitSpeed() {
        ...
        if(speed>1250) { vx*=1250/speed;vy*=1250/speed; }
    }
    ...
    private void flipper(boolean isLeft) {
        ...
        if(segment(rail,FLIPPER_RADIUS,-dy*t*omega,dx*t*omega,.68,0) && Math.abs(omega)>1) flipperHits++;
    }
